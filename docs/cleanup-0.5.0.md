...
- [x] **P6.2 — Blocking sync IO on async paths** · Medium · ✅ — *done: the two latency-sensitive paths now run their recursive `read_dir` + TOML loads inside `tokio::task::spawn_blocking`. The per-keystroke `@reference` autocomplete (`suggestions.rs`) — the path the finding called out — moves its `load_vault_reference_entries` off the runtime; `build_reference_context` (the generation + reroll `@reference` assembly) became `async` and spawn-blocks its whole vault scan (callers in `ai_generation`/`entity_reroll`'s `resolve_reference_suffix` now `.await`). Chose spawn_blocking over caching to avoid invalidation complexity near release. The remaining `EntityStore` reads sit only on the startup boot task and user-initiated saves (not the per-keystroke path) — acceptable blocking for a single-user desktop app; left as-is.*

- [x] **P6.3 — `vault_sync` is store→db only, not vault↔db** · Medium · ✅ — *done (user chose **fix naming/docs + atomic reap**, accepting the store as the source of truth): renamed `VaultSyncService::sync_from_vault` → `project_store_into_db` with a doc stating the direction is store → db only and the Obsidian `.md` vault is a publish target, not an input; corrected the "reconcile" framing in the module comments + `docs/architecture.md`. The reap/prune is now transactional (landed with P6.1). The disk-scan half was intentionally **not** built.*

- [x] **P6.4 — `ollama_chat` ignores truncation / 200-with-error** · High · ✅ — *done: extracted `interpret_chat_response` (pure, unit-tested) from `post_chat_for_content`; a top-level `{"error": …}` body and `done_reason == "length"` now abort with an actionable message (raise `num_ctx` / lower verbosity) instead of being swallowed into the retry loop's generic failure. Missing content still → `Ok(None)` (retryable), content passes through — the `Result<Option<String>>` contract is unchanged, so every retry-loop call site is untouched. `attempt_seed`'s intentional `i32` truncation is now commented. 4 new tests.*
...
- [x] **P6.5 — `link_prose` re-allocates per (position × name)** · Low · ✅ — *done: `EntityLinker` now stores `(canonical, lowercased)` pairs, computing each lowercased needle once in `new` (it was already built there for de-dup and discarded) instead of re-allocating it per text-position × name in the inner loop.*

- [x] **P6.6 — `resources_assets` parsed by heuristic** · Low · ✅ — *done (with user decision: model-first): `resources_assets` is now `Vec<String>` like the other faction list fields — array generation schema/seed, `normalize_unknown_list` on seed/draft/save, JSON text in the DB column (TEXT) via `faction_list_to_db_text`, `ValueKind::List` (CSV set + list-value reroll), plain bulleted publish section. The reverse-engineering heuristic (`parse_text_list_items` + `write_text_list_section`) is deleted. Legacy string-form TOML still loads via a lenient `string_or_seq` deserializer on `FactionFrontmatter` (splits a scalar on `, ; \n` once at the I/O boundary); `FactionDraft` keeps a clean `Array<string>` ts-rs contract. `models.ts` regenerated.*
