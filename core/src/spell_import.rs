...
//!
//! The only fiddly parsing is the 5etools inline-markup handling in
//! [`crate::fivetools_markup`] (`strip_tags` / `render_inline`), which lowers tags
//! like `{@damage 8d6}` / `{@condition Prone|XPHB}` to display text or clickable
//! spans. That seam is shared with the monster importer; everything here is
//! mechanical field mapping on top of it.
...
use crate::fivetools_markup::{
    coalesce_spans, render_inline, slugify, spans_non_empty, strip_tags,
};
        ...
        match entry {
            Entry::Text(text) => {
                ...
                if spans_non_empty(&spans) {
                    out.push(SpellBlock::Text { spans });
                }
            }
...
/// Flatten one list item to a single bullet line of spans (cross-links survive). A
/// named item (`{type:"item", name, entries}`) renders as "Name. body".
fn lower_list_item(item: &ListItem) -> Vec<Span> {
    match item {
        ListItem::Text(text) => render_inline(text),
        ListItem::Item(block) => {
            ...
            for entry in &block.entries {
                if let Entry::Text(text) = entry {
                    ...
                    body.extend(render_inline(text));
                }
            }
...
mod tests {
    ...
    fn list_spell_lowers_named_items_to_bullets() {
        ...
        assert!(bullets[1].contains("Prone")); // {@condition Prone} lowered to a link label
    }
